    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        def closure():
            ...
            if sglr_avoid == True:
                ...
                train_loss = loss_fn(pred[id_], dataset['train_label'][train_id][id_].to(device).long())# Добавил .long()
            else:
                train_loss = loss_fn(pred, dataset['train_label'][train_id].to(device).long()) # Добавил .long()
