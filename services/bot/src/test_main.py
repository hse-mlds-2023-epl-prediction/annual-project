import pytest
from main import games_today, games_tomorrow, games_ten, tomorrow_predict, \
    today_predict, ten_predict, stats
...
async def test_games_today(mocker: MockerFixture):
    ...
    requester = MockedBot(request_handler=MessageHandler(
        games_today, auto_mock_success=False))
...
async def test_games_tomorrow(mocker: MockerFixture):
    mocker.patch('main.make_request', return_value=[])
    requester = MockedBot(
        request_handler=MessageHandler(games_tomorrow, auto_mock_success=False)
        )
...
async def test_games_ten(mocker: MockerFixture):
    mocker.patch('main.make_request', return_value=[])
    requester = MockedBot(
        request_handler=MessageHandler(games_ten, auto_mock_success=False)
        )
    ...
    mocker.patch('main.make_request', return_value=[{'Home': '1',
                                                     'Away': '2',
                                                     'Ground': '3'}])
    requester = MockedBot(
        request_handler=MessageHandler(games_ten, auto_mock_success=False)
...
async def test_tomorrow_predict(mocker: MockerFixture):
    ...
    requester = MockedBot(
        request_handler=MessageHandler(
            tomorrow_predict, auto_mock_success=False
            )
        )
...
async def test_ten_predict(mocker: MockerFixture):
    mocker.patch('main.make_request', return_value=[])
    requester = MockedBot(
        request_handler=MessageHandler(ten_predict, auto_mock_success=False)
        )
...
async def test_today_predict(mocker: MockerFixture):
    mocker.patch('main.make_request', return_value=[])
    requester = MockedBot(
        request_handler=MessageHandler(today_predict, auto_mock_success=False)
        )
...
async def test_stats(mocker: MockerFixture):
    mocker.patch('main.make_request', return_value=[])
    requester = MockedBot(
        request_handler=MessageHandler(stats, auto_mock_success=False)
        )
