...
with tab1:
    ...
    df_player = df_pl[
        (df_pl['season'] >= season_player[0]) & (df_pl['season'] <= season_player[1])]  # Датасет по нужным сезонам
    df_player_gr = df_player.groupby(by='name_display', as_index=False).sum().sort_values(
...
with tab2:
    ...
    df_club = df_cl[
        (df_cl['season'] >= season_player[0]) & (df_cl['season'] <= season_player[1])]  # Датасет по нужным сезонам
    df_club_gr = df_club.groupby(by='club_name', as_index=False).sum().sort_values(by=dict_feat_agg_club[feat_club_agg],
                                                                                   ascending=False)
...
with tab3:
    ...
    df_games = df_gm[
        (df_gm['gameweek_compSeason_label'] >= season_player[0]) & (df_gm['gameweek_compSeason_label'] <= season_player[1])
    ]  # Датасет по нужным сезонам
    df_games_temp = df_games[
        ((df_games['teams_team_1_name'] == list_club_game[0]) & (df_games['teams_team_2_name'] == list_club_game[1])) |
    ...
    df_games_temp.rename(columns={'teams_team_1_name': 'Команда (h)', 'teams_team_2_name': 'Команда (g)',
                                  'team_1_wins': 'Победы команды (h)', 'team_2_wins': 'Победы команды (g)',
    ...
    df_games['team_1_hue'] = np.where(df_games['team_1_hue'] == 1, 'Победа',
    ...
    if select_box is not None:
        fig = plt.figure()
        ax = sns.scatterplot(data=df_games, x=feature_dict[map_feature_dict[select_box]][0],
                             y=feature_dict[map_feature_dict[select_box]][1], hue='Результат матча')
