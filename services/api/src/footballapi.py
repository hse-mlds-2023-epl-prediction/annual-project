...
def get_dataframe():
    ...
    for game in r:
        ...
        games['gameweek_compSeason_label'].append(
            game['gameweek']['compSeason']['label'][:4])
    ...
    df['gameDate'] = df['gameDate'].apply(
        lambda x: ' '.join(x.replace(',', '').split()[1:4][::-1]))
...
def prepare(df):
    df_mean = pd.read_csv(
        os.path.join(os.path.dirname(__file__)) + '/model/df_mean.csv'
    )
    df_lag = pd.read_csv(
        os.path.join(os.path.dirname(__file__)) + '/model/df_lag.csv'
    )
    df = df.merge(
        df_mean, left_on='teams_team_1_name', right_on='club_name', how='left'
    )
    df = df.merge(
        df_mean, left_on='teams_team_2_name', right_on='club_name', suffixes=(
            '_team_1', '_team_2'
            ), how='left'
    )
    df['gameweek_compSeason_label'] = df['gameweek_compSeason_label'].astype(
        'int')
    df = pd.merge(df,
                  df_lag,
    ...
    df.drop(
        ['season_lag_team2',
         ...
         'club_name_lag_team1',
         'club_name_lag_team2',
         'gameDate'], axis=1, inplace=True
        )

    with open(os.path.join(
            os.path.dirname(__file__)) + '/model/catboost.pickle', 'rb') as f:
        model = pickle.load(f)

    with open(os.path.join(
            os.path.dirname(__file__)) + '/model/name_cols.pickle', 'rb') as f:
...
def helthy_services(services: dict = uri):
    result = {}
    for name, uri_ in services.items():
        result[name] = helthy_service(uri_)
    return result
