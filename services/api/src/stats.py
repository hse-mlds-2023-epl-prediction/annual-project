...
async def stats():
    df = pd.read_csv(os.path.join(
        os.path.dirname(__file__)) + '/data/2023_2024_matches.csv')
    ...
    avg_yellow_cards_count_home = df.groupby(
        'home_team_name')['home_team_yellow_cards_count'].mean()
    avg_yellow_cards_count_away = df.groupby(
        'away_team_name')['away_team_yellow_cards_count'].mean()
